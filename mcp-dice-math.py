...
def all_damage_probabilities():
    ...
    for damage in range(1, num_atk_dice * 2 + 1):
        gt_damage_proba = gt_cumulative_damage_proba(
            atk_dice, def_dice, num_atk_dice, num_def_dice, damage)
        output.insert(tk.INSERT, (str(damage) + '+: ' +
                                  "{:.2f}".format(gt_damage_proba) + '\n'))
    ...
    if damage > 0:
        for defense in range(0, num_def_dice * 2 + 1):
            if (defense + damage) <= num_atk_dice * 2:
                proba += roll_proba(atk_dice, num_atk_dice, defense +
                                    damage) * roll_proba(def_dice, num_def_dice, defense)

    else:
        for defense in range(0, num_def_dice * 2 + 1):
            for attack in range(0, defense + 1):
                proba += roll_proba(atk_dice, num_atk_dice, attack) * \
                    roll_proba(def_dice, num_def_dice, defense)
...
def gt_cumulative_damage_proba(atk_dice, def_dice, num_atk_dice, num_def_dice, damage):
    ...
    for i in range(0, damage):
        cumulative_proba += damage_probabilities(
            atk_dice, def_dice, num_atk_dice, num_def_dice, i)
...
def create_options_window():
    ...
    tk.Label(options_window, text='Successful Faces (Attack)').grid(
        row=0, column=0)
    tk.Label(options_window, text='Successful Faces (Defense)').grid(
        row=0, column=2)

    tk.Checkbutton(options_window, text='Hit',
                   variable=atk_cb_hit).grid(row=1, column=0)

    tk.Checkbutton(options_window, text='Block',
                   variable=atk_cb_block).grid(row=2, column=0)

    tk.Checkbutton(options_window, text='Wild',
                   variable=atk_cb_wild).grid(row=3, column=0)

    tk.Checkbutton(options_window, text='Crit',
                   variable=atk_cb_crit).grid(row=4, column=0)

    tk.Checkbutton(options_window, text='Blank',
                   variable=atk_cb_blank).grid(row=5, column=0)

    tk.Checkbutton(options_window, text='Fail',
                   variable=atk_cb_fail).grid(row=6, column=0)

    tk.Checkbutton(options_window, text='Hit',
                   variable=def_cb_hit).grid(row=1, column=2)

    tk.Checkbutton(options_window, text='Block',
                   variable=def_cb_block).grid(row=2, column=2)

    tk.Checkbutton(options_window, text='Wild',
                   variable=def_cb_wild).grid(row=3, column=2)

    tk.Checkbutton(options_window, text='Crit',
                   variable=def_cb_crit).grid(row=4, column=2)

    tk.Checkbutton(options_window, text='Blank',
                   variable=def_cb_blank).grid(row=5, column=2)

    tk.Checkbutton(options_window, text='Fail',
                   variable=def_cb_fail).grid(row=6, column=2)

    tk.Button(options_window, text='Update Dice',
              command=update_dice).grid(row=7, column=1)
...
num_atk_dice.set(1)
atk_dice_dd = tk.OptionMenu(
    window, num_atk_dice, *arange(1, 13, 1), command=store_current_atk_dice_num)
atk_dice_dd.grid(row=1, column=1)

num_def_dice = tk.IntVar(window)
num_def_dice.set(1)
def_dice_dd = tk.OptionMenu(
    window, num_def_dice, *arange(1, 9, 1), command=store_current_def_dice_num)
